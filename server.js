...
// Your CF Worker base (no trailing slash)
const WORKER = process.env.WORKER_ORIGIN || 'https://vod.mmonterrosa970.workers.dev'
    ...
    try {
      ...
      sources.push({
        ...
        proxyUrl:
          entry.type === 'mp4'
            ? null
            : `${WORKER}/?proxy=${encodeURIComponent(url)}`,
      })
    } catch {
      // skip bad entry
    }
...
app.get('/resolve', async (req, res) => {
  try {
    ...
    if (!parsed) {
      return res.status(400).json({ error: 'Could not parse id' })
    }
    const { parsed: p, sources } = await resolveSources(parsed)
    if (!sources.length) {
      return res.status(404).json({
        error: 'No full-length streams (only decoys or empty)',
        tmdbId: p.id,
      })
    }
    res.json({
